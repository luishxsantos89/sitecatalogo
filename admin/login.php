...
<<<<<<< HEAD
            ...
            if ($user) {
                // Verifica se é hash bcrypt ou texto plano
                $senha_ok = false;
                
                if (password_verify($senha, $user['senha'])) {
                    // É um hash bcrypt válido
                    $senha_ok = true;
                } elseif ($user['senha'] === $senha) {
                    // É texto plano (compatibilidade temporária)
                    $senha_ok = true;
                    
                    // Atualiza para hash automaticamente (migração)
                    $novo_hash = password_hash($senha, PASSWORD_DEFAULT);
                    db()->prepare("UPDATE " . table('usuarios') . " SET senha = ? WHERE id = ?")
                ...
                if ($senha_ok) {
                    ...
                    log_activity('login', 'auth', "Usuario {$user['email']} fez login");
